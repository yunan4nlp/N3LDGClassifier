...
public:
	bool initial(HyperParams& opts, AlignedMemoryPool* mem = NULL){
		...
		hidden_linear_layers[0].initial(opts.hiddenSize, opts.windowOutput_layer1, true, mem);
		int cnnLayerSize = hidden_linear_layers.size();
		for (int idx = 1; idx < cnnLayerSize; idx++) {
			hidden_linear_layers[idx].initial(opts.hiddenSize, opts.windowOutput_layer2, true, mem);
		}
