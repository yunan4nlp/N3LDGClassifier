...
public:
	bool initial(HyperParams& opts, AlignedMemoryPool* mem = NULL){
		...
		opts.windowOutput = (opts.wordDim + opts.charHiddenSize) * opts.wordWindow;
		...
		char_linear.initial(opts.charHiddenSize, opts.charWindowOutput, true, mem);
	...
	void exportModelParams(ModelUpdate& ada){
		...
		char_linear.exportAdaParams(ada);
	...
	void exportCheckGradParams(CheckGrad& checkgrad){
		checkgrad.add(&chars.E, "charE");
		checkgrad.add(&words.E, "wordE");
	}
